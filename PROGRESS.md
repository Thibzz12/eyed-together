...
- **Date réelle des événements — recherche effectuée, bloqué côté WordPress (pas un bug de l'app) :**
  ...
  - **⏳ EN ATTENTE (2026-07-23)** : Thibaud a vérifié un premier groupe de champs ACF ("Block : Événements") — celui-ci ne contient que "Titre" et "Bouton" (un bloc Gutenberg d'affichage, pas les données de l'événement lui-même) : pas le bon groupe, pas de champ date dedans. Il doit chercher/demander demain le VRAI groupe de champs rattaché au **type de contenu** `evenement` (pas un bloc de page) — c'est probablement là que vit le champ date. Si aucun groupe de champs ACF n'est rattaché au type de contenu lui-même, il se peut que la date ne soit pas structurée du tout côté WordPress (juste écrite dans le texte) — à confirmer avec quelqu'un qui gère le site.
